...
<template>
  <div class="space-y-6 max-w-lg">
    <section class="bg-white dark:bg-ink-700 rounded-card border border-ink-100 dark:border-ink-500 shadow-card p-5">
      <h2 class="text-[15px] font-semibold mb-4">Pengaturan Akun</h2>
      <form class="space-y-3" @submit.prevent="simpan">
        <div>
          <label class="block text-[13px] font-medium mb-1">Nama</label>
          <input v-model="akun.nama" type="text" class="w-full rounded-lg border border-ink-100 px-3 py-2.5 text-[13.5px]" />
        </div>
        <div>
          <label class="block text-[13px] font-medium mb-1">Email</label>
          <input v-model="akun.email" type="email" class="w-full rounded-lg border border-ink-100 px-3 py-2.5 text-[13.5px]" />
        </div>
    ...
    <section class="bg-white dark:bg-ink-700 rounded-card border border-ink-100 dark:border-ink-500 shadow-card p-5">
      <h2 class="text-[15px] font-semibold mb-4">Tampilan</h2>
      <div class="flex items-center justify-between">
        <p class="text-[13.5px]">Mode {{ isDark ? 'Gelap' : 'Terang' }}</p>
        <button
          type="button"
          class="w-12 h-6 rounded-full flex items-center px-0.5 transition-colors"
          :class="isDark ? 'bg-brand-500 justify-end' : 'bg-ink-100 justify-start'"
          @click="toggle"
        >
