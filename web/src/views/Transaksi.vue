  ...
  <div>
    ...
    <div class="bg-white dark:bg-ink-700 rounded-card border border-ink-100 dark:border-ink-500 shadow-card overflow-hidden">
      <table class="w-full text-left text-[13.5px]">
        ...
        <tbody>
          <tr v-for="p in daftar" :key="p.id" class="border-b border-ink-100 dark:border-ink-500 last:border-0 dark:text-ink-100">
            ...
            <td class="px-4 py-3 text-right">
              <button type="button" class="text-danger-600 dark:text-danger-500 text-[12.5px] font-semibold" @click="hapus(p.id)">Hapus</button>
            </td>
          </tr>
          <tr v-if="daftar.length === 0">
            <td colspan="7" class="px-4 py-6 text-center text-[13px] text-ink-500 dark:text-ink-300">Belum ada transaksi penjualan.</td>
          </tr>
        </tbody>
      </table>
    </div>
