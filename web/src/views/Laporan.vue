      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          <select
            ...
            class="text-[13.5px] border border-ink-100 dark:border-ink-500 rounded-lg px-3 py-2 bg-white dark:bg-ink-900 dark:text-white"
          >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="bg-white dark:bg-ink-700 rounded-card border border-ink-100 dark:border-ink-500 shadow-card p-5">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-[12px] px-2 py-0.5 rounded-full bg-brand-100 text-brand-700 dark:bg-brand-600/25 dark:text-brand-100 font-medium">
              {{ namaBulanDipilih }}
            </span>
          </div>
          <div class="space-y-3 text-[13.5px]">
            <div class="flex justify-between">
              <span class="text-ink-500 dark:text-ink-300">Panen (ekor)</span>
              <span class="font-semibold dark:text-ink-100">{{ data.total_panen_ekor || 0 }} ekor</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-500 dark:text-ink-300">Panen (kg)</span>
              <span class="font-semibold dark:text-ink-100">{{ data.total_panen_kg || 0 }} kg</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-500 dark:text-ink-300">Penjualan</span>
              <span class="font-semibold text-brand-600 dark:text-brand-400">{{ rupiah(data.total_penjualan) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-500 dark:text-ink-300">Pengeluaran</span>
              <span class="font-semibold text-danger-600 dark:text-danger-500">{{ rupiah(data.total_pengeluaran) }}</span>
            </div>
        ...
        <div class="bg-white dark:bg-ink-700 rounded-card border border-ink-100 dark:border-ink-500 shadow-card p-5">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-[12px] px-2 py-0.5 rounded-full bg-ink-100 text-ink-600 dark:bg-ink-500/40 dark:text-ink-200 font-medium">
              {{ namaBulanSebelumnya }}
            </span>
          </div>
          <div class="space-y-3 text-[13.5px]">
            <div class="flex justify-between">
              <span class="text-ink-500 dark:text-ink-300">Panen (ekor)</span>
              <span class="font-semibold dark:text-ink-100">{{ data.bulan_lalu?.panen_ekor || 0 }} ekor</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-500 dark:text-ink-300">Panen (kg)</span>
              <span class="font-semibold dark:text-ink-100">{{ data.bulan_lalu?.panen_kg || 0 }} kg</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-500 dark:text-ink-300">Penjualan</span>
              <span class="font-semibold text-brand-600 dark:text-brand-400">{{ rupiah(data.bulan_lalu?.penjualan) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-ink-500 dark:text-ink-300">Pengeluaran</span>
              <span class="font-semibold text-danger-600 dark:text-danger-500">{{ rupiah(data.bulan_lalu?.pengeluaran) }}</span>
            </div>
            <div class="border-t border-ink-100 dark:border-ink-500 pt-3 flex justify-between">
              <span class="font-medium dark:text-white">Keuntungan</span>
