...
<template>
  ...
  <div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center px-4">
    ...
    <div class="relative bg-white dark:bg-ink-700 rounded-card shadow-card w-full max-w-md p-5">
      <h2 class="text-[16px] font-semibold mb-4">Catat Sortir — {{ jadwalDipilih?.nama_kolam }}</h2>
      <form class="space-y-3" @submit.prevent="simpan">
        ...
        <div>
          <label class="block text-[13px] font-medium mb-1">Jumlah Ikan Mati</label>
          <input v-model="form.jumlah_mati" type="number" min="0" required class="w-full rounded-lg border border-ink-100 px-3 py-2.5 text-[13.5px]" />
        </div>
        <div>
