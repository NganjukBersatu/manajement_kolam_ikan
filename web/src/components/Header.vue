...
defineProps({
  onOpenSidebar: { type: Function, default: null }
})
...
<template>
  <header class="h-14 sm:h-16 flex items-center gap-2 sm:gap-3 px-3 sm:px-5 bg-white dark:bg-ink-900 border-b border-ink-100 dark:border-ink-700 shrink-0">
    <!-- Hamburger (mobile only) -->
    <button
      v-if="onOpenSidebar"
      type="button"
      class="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg text-ink-600 dark:text-white hover:bg-ink-100 dark:hover:bg-ink-700 transition shrink-0"
      @click="onOpenSidebar"
    ...
    <div class="min-w-0 flex-1">
      <h1 class="text-[15px] sm:text-[17px] font-semibold text-ink-900 dark:text-white truncate">
        {{ route.meta.title || 'Dashboard' }}
      </h1>
      <p v-if="route.meta.subtitle" class="text-[11px] sm:text-xs text-ink-400 dark:text-ink-300 mt-0.5 truncate hidden sm:block">
        {{ route.meta.subtitle }}
      </p>
    </div>
    ...
    <div class="flex items-center gap-1.5 sm:gap-3 shrink-0">
      ...
      <div class="relative" ref="notifRef">
        <button
          type="button"
          class="relative w-9 h-9 flex items-center justify-center rounded-lg text-ink-600 dark:text-white hover:bg-ink-100 dark:hover:bg-ink-700 transition"
          @click="toggleNotif"
        ...
        <div
          v-if="notifOpen"
          class="absolute right-0 top-[calc(100%+8px)] w-[min(20rem,calc(100vw-1.5rem))] bg-white dark:bg-ink-800 border border-ink-100 dark:border-ink-700 rounded-card shadow-card py-2 z-50"
        >
          <div class="flex items-center justify-between px-4 py-2 border-b border-ink-100 dark:border-ink-700">
            ...
            <div class="flex items-center gap-2">
              <button v-if="unreadCount > 0" type="button" class="text-[12px] text-brand-500 hover:text-brand-600 font-medium" @click="markAllAsRead">
                Tandai dibaca
              </button>
              <button v-if="notifications.length > 0" type="button" class="text-[12px] text-ink-400 hover:text-danger-500 font-medium" @click="clearAll">
                Bersihkan
              </button>
            </div>
          </div>
      ...
      <button
        type="button"
        class="w-9 h-9 flex items-center justify-center rounded-lg text-ink-600 dark:text-white hover:bg-ink-100 dark:hover:bg-ink-700 transition"
        @click="toggle"
      ...
      <div class="relative" ref="menuRef">
        <button
          type="button"
          class="flex items-center gap-2 pl-2 sm:pl-3 border-l border-ink-100 dark:border-ink-700 hover:bg-ink-100 dark:hover:bg-ink-700 rounded-lg pr-1.5 sm:pr-2 py-1 transition"
          @click="toggleMenu"
        >
          <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-brand-500 flex items-center justify-center text-white font-semibold text-sm shrink-0 overflow-hidden">
          ...
          <svg class="hidden sm:block text-ink-400" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        >
          ...
          <div class="py-1">
            <button type="button" class="w-full flex items-center gap-2.5 px-4 py-2 text-[13.5px] text-ink-700 dark:text-ink-100 hover:bg-ink-100 dark:hover:bg-ink-700 transition" @click="goToProfile">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
              Lihat profil
            </button>
            <button type="button" class="w-full flex items-center gap-2.5 px-4 py-2 text-[13.5px] text-ink-700 dark:text-ink-100 hover:bg-ink-100 dark:hover:bg-ink-700 transition disabled:opacity-50" :disabled="uploading" @click="triggerFileInput">
